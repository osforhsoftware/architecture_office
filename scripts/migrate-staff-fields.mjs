...
const migrationPath = path.join(root, "scripts", "migrate-staff-fields.sql")
...
try {
  console.log("Applying staff fields migration (MySQL)...")
  const ddl = fs.readFileSync(migrationPath, "utf8")
  await execRaw(ddl)
  console.log("Staff fields migration applied successfully.")
} catch (error) {
  console.error("Failed to apply staff fields migration:", error.message)
  process.exitCode = 1
} finally {
  await pool.end()
}
