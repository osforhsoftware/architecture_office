...
const migrationPath = path.join(root, "scripts", "migrate-attendance.sql")
...
try {
  console.log("Applying attendance migration (MySQL)...")
  const ddl = fs.readFileSync(migrationPath, "utf8")
  await execRaw(ddl)
  console.log("Attendance migration applied successfully.")
} catch (error) {
  console.error("Failed to apply attendance migration:", error.message)
  process.exitCode = 1
} finally {
  await pool.end()
}
